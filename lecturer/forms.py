...
from core.models import Question, Faculty, Department, STATES, Course, Tma, Topic
...
class TmaForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["course"].queryset = Course.objects.filter(lecturer=self.request.user.lecturer)


class QuestionForm(forms.ModelForm):
    text = forms.CharField(required=True,
                           widget=forms.Textarea(attrs={'rows': 2, 'placeholder': 'Enter your Question.'}),
                           max_length=4000,
...
class NewTopicForm(forms.ModelForm):
    subject = forms.CharField(required=True, widget=forms.TextInput(attrs={'placeholder': 'Add Subject'}))
    ...
    def __init__(self, *args, **kwargs):
        ...
        super().__init__(*args, **kwargs)
        self.fields["course"].queryset = Course.objects.filter(lecturer=self.request.user.lecturer)
        self.fields['message'].widget.attrs.update({'placeholder': 'Choose A Unique Username','row': 8})
